...
def train_update_kb(model, train_dl, val_dl, criterions, task_id, lr, device=torch.device('cuda')):
    ...
    for epoch in epoch_bar:
        ...
        for i, (img, label, lengths) in enumerate(train_dl):
            if i > k:
                break
            img = img.to(device)
            ...
            with torch.no_grad():
                ...
                img_val, label_val, lengths = next(iter(val_dl))
                # TODO: Modify acc for each classifier differently
                loss, acc = train_batch(model_cp, img_val, label_val, lengths, None, criterions, task_id, 1, device)
                model_cp.train()
...
def train_batch(model, img, label, lengths, optimizer, criterions, task_id, num_epochs, device = torch.device('cuda')):
    ...
    for epoch in range(num_epochs):
        if optimizer is not None:
            optimizer.zero_grad()
        logits = model(img, lengths, task_id)
        ...
        for i in range(4):
            acc_meters[i].update(accs[i], img.shape[0])
        # epoch_bar.set_postfix(loss=loss.item(), accuracy=get_acc(logits,label).item())
        # epoch_bar.set_description(f'Epoch: {epoch + 1}')
        # epoch_bar.update()
    return loss_meter.avg, sum([a.avg for a in acc_meters.values()])/4
