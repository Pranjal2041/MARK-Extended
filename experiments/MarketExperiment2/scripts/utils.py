...
def lstm_loss(criterions, logits, labels):
    return sum([criterions[i](logits[i], labels[:,i]) for i in range(labels.shape[1])])/logits[0].shape[0]

def lstm_accuracy(logits, labels):
    return {i:(torch.max(logits[i], 1)[1] == labels[:, i]).float().mean().item() for i in range(labels.shape[1])}

def lstm_f1(metric, logits, labels):
    {i:metric[i](logits[i].cpu(), labels[:,i].cpu()) for i in range(len(logits))}
