...
def main(cfg, experimenter):
    ...
    print(f'Final Accuracy: {accs.mean(axis = 1)} ( {accs.std(axis = 1)} )', priority = LogPriority.STATS)
    print(f'Final BWT: {bwts.mean(axis = 1)} ( {bwts.std(axis = 1)} )', priority = LogPriority.STATS)
