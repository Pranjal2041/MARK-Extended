...
def train(model : MARKLSTMModel, criterion, train_dl, experimenter, device):
    ...
    for day in range(NUM_DAYS-1): # Last Day only for testing?
        ...
        for task_id in range(NUM_SYMBOLS):
            ...
            loss, acc = train_fe(model, train_dl, criterion, task_id, device = device, lr = float(experimenter.config.TRAINING.LR.FE), num_epochs = experimenter.config.TRAINING.EPOCHS.FE)
            ...
            if task_id == 0:
                ...
                loss, acc = train_kb_nonmeta(model, train_dl, criterion, task_id, lr = float(experimenter.config.TRAINING.LR.INIT_KB), device = device, num_epochs = experimenter.config.TRAINING.EPOCHS.INIT_KB)
            ...
            loss, acc = train_mg_n_clf(model, train_dl, criterion, task_id, lr = float(experimenter.config.TRAINING.LR.MG_N_C), device = device, num_epochs = experimenter.config.TRAINING.EPOCHS.MG_N_C)
            ...
            # update_kb(model, train_dl, train_dl, criterion, task_id, device=device)
            ...
            for t_id in range(task_id+1):
                ...
                loss, acc = test_loop(model, train_dl, t_id, device = device)
