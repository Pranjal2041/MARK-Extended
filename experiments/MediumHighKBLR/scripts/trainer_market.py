...
def train_mg_n_clf(model : MARKModel, dl, criterion, task_id, lr = 1e-2, num_epochs = 50, device = torch.device('cuda')):
    ...
    return train_loop(model, dl, optimizer, criterion, task_id, num_epochs, device)
...
def train_fe(model : MARKModel, dl, criterion, task_id, lr = 1e-1, num_epochs=50, device = torch.device('cuda')):
    ...
    return train_loop(model_fn, dl, optimizer, criterion, task_id, num_epochs = num_epochs, device = device)
...
def train_loop(model : Union[MARKModel,Callable[[torch.tensor, int], torch.tensor]], dl, optimizer, criterion, task_id, num_epochs=50, device = torch.device('cuda')):
    ...
    for epoch in epoch_bar:
        ...
        for _, (inp_feats, label, lengths) in enumerate(dl):
            ...
            optimizer.zero_grad()
            logits = model(inp_feats, task_id)
            loss = model.loss(criterion, logits, label)
            if (loss.item() > 100):
                print("Prob is here")
            loss.backward()
            for i in range(len(optimizer.param_groups)):
            ...
            accs = model.accuracy(logits,label)
            for i in range(4):
                acc_meters[i].update(accs[i], inp_feats.shape[0])
        epoch_bar.set_postfix(loss = loss_meter.avg, A0 = acc_meters[0].avg, A1 = acc_meters[1].avg, A2 = acc_meters[2].avg, A3 = acc_meters[3].avg)
        epoch_bar.set_description(f'Epoch: {epoch+1}')
        epoch_bar.update()
