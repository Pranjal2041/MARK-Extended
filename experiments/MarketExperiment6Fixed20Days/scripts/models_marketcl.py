...
class FeatureExtractorLSTMClassifier(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, num_labels = 4, supsup = False, num_tasks = -1):
        ...
        self.fcs = nn.ModuleList([nn.Linear(input_dim, output_dim) if not self.supsup else MultitaskMaskLinear(input_dim, output_dim, num_tasks = self.num_tasks, bias = False) for _ in range(self.num_labels)])
