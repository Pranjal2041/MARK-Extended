...
class MARKModel(nn.Module):
    ...
    def __init__(self, FE_HIDDEN : int, EMBED_DIM : int, DIMENSIONS : List[int], NUM_CLASSES : int, NUM_TASKS : int, device : torch.device):

        super().__init__()
        self.fes = list_to_device([FeatureExtractor(3, FE_HIDDEN, EMBED_DIM) for _ in range(NUM_TASKS)], device)
        self.fecs = list_to_device([FeatureExtractorClassifier(EMBED_DIM, NUM_CLASSES) for _ in range(NUM_TASKS)], device)
        self.mgs = list_to_device([MaskGenerator(EMBED_DIM, DIMENSIONS) for _ in range(NUM_TASKS)], device)
        self.kb = KB((3,32,32), EMBED_DIM, DIMENSIONS).to(device)
        self.kbcs = list_to_device([KBClassifier(EMBED_DIM, 5) for _ in range(NUM_TASKS)], device)
